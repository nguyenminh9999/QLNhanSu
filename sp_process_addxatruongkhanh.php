...
if(( $macb == "") || ($hoten == ""))
{
	...
	window.location = "sp_addxatruongkhanh.php";
	</script>
	<?php
}
else
{
		include("ketnoi.inc");
		$ketqua_ktra = mysql_query("select MaCB from nhansuxatruongkhanh where MaCB = '{$macb}'",$link);
		$num = mysql_num_rows($ketqua_ktra);
		if( $num == 0 ) // Kiểm tra trùng dữ liệu
		{
				$ketqua_them = mysql_query("insert into nhansuxatruongkhanh (MaCB,Hoten,Ngaysinh,Gioitinh,Quoctich,Dantoc,Tongiao,Quequan,Giaoducphothong,Chuyenmonnghiepvu,Tuoi,Lyluanchinhtri,Ngoaingu,Nghenghiepchucvu,Noicongtac,Ngayvaodang, Ghichu,Thaotac) values('$macb','$hoten','$ngaysinh','$gioitinh','$quoctich','$dantoc','$tongiao','$quequan','$giaoducphothong','$chuyenmonnghiepvu','$tuoi','$lyluanchinhtri','$ngoaingu','$nghenghiepchucvu','$noicongtac','$ngayvaodang','$ghichu','$thaotac')",$link);
				if($ketqua_them)
				{
					
					header("location:sp_chitietxatruongkhanh.php?MaCB='$macb'"); 
				}
				else
				{
					?>
